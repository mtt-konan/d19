...
| C5 | Reduced-pair safe conditions `A` odd, `B` odd, `(A+B)%4==0` are safe only for reduced/coprime full-chain setting. | `CURRENT_FINDINGS.md:51-71`, `MATH.md:439-443`, `safe_pair_sieve.py:1-23`, `methods.py:103-143` | proved under coprime input; wrapper skips non-coprime input | coprime mod-12 theorem | `safe_sieve` | `tests/test_coprime_mod12.py`, `tests/test_proof_status.py`, focused tests pass | If future code removes the guard or treats `safe_sieve=skipped` as proof. |
...
|---|---|---|---|---|
| S1 | `safe_sieve` / `classify_reduced_pair` | proved under coprime input; `run_safe_sieve` skips non-coprime input | `MATH.md:408-443`, `safe_pair_sieve.py:1-23`, `methods.py:103-143`, `tests/test_proof_status.py` | reduced/coprime only; arbitrary pairs continue to gcd-aware/full-plane methods |
