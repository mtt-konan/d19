...
- H1. `proof_status --pair` can feed arbitrary non-coprime input into a coprime-only `safe_sieve`, then store a terminal `no_solution` label.
  ...
  Pre-fix smoke check: `(6,15)` is non-coprime, but `run_safe_sieve(6,15)` returned `no_solution` with `classification='mixed_parity'`. That exact pair is also killed by the newer gcd-aware condition, so it is not a false result example; it is evidence that the coprime-only method was reachable without its precondition.

  Post-fix note (2026-06-09): `run_safe_sieve(6,15)` now returns `skipped` with `classification='not_reduced_coprime'`; workflow continues to later full-plane/gcd-aware methods. See wl125.
