...
| safe-pair / modular / gcd-aware sieves | active supporting tools | `safe_sieve` is sound for coprime reduced input and now skips non-coprime input at the proof-status wrapper; `gcd_aware_kills` generalizes to arbitrary `(A,B)` as a necessary filter; full-plane mod p^2 is implemented. | Keep coprime-only and arbitrary-pair filters separate; do not reinterpret `safe_sieve=skipped` as proof. |
