...
**实测数据（max_hyp=100，254 个 reduced pair）**：

- `no_solution` 严格证明：238（93.7%）
  - 由 safe_sieve 砍掉：231
  - 由 factor_concordant 砍掉：7
- `hard_case`（pipeline 用尽，需更深方法）：16
...
对 `max_hyp=100` 的全部 16 个 hard_case，实测 `lower == upper`（10 个 rank=1，6 个 rank=2）。
...
|------|------|---------|---------|---------|---------|
| 一 | 因子分解攻击 ✅已实现 | 低 | 中 | 高 | — |
| 二 | Gaussian 整数 mod $p^2$ 筛 | 中 | 低 | 中高 | **下一步** |
| 三 | 标准 2-descent / Selmer | 高 | 高 | 高 | 中期 |
| 四 | 对角符号筛加强 | 低 | 低 | 中 | 可选 |
| 五 | **Heegner 点构造** | 中 | 低 | **高** | **优先尝试** |
| 六 | L-函数高阶导数 | 中 | 低 | 低 | PARI 已精确算 rank，此方向降级 |
| 七 | Chabauty / Quadratic Chabauty | 高 | 高 | **极高** | 长期攻关 |
| 八 | Brauer–Manin 障碍 | 极高 | 极高 | **极高** | 学术合作 |
| 九 | Second descent | 高 | 中 | 中 | 中期 |
...
如果你不是椭圆曲线/代数几何专家，直接动手方向七、八、十不现实。但下面这条路是可行的：

1. **先做方向五（Heegner 点）**
   - 学习成本：读完 Silverman《Advanced Topics in the Arithmetic of Elliptic Curves》第 IV-V 章 + Sage `EllipticCurve` 文档
   - 工程量：写一个 `concordant --method heegner` 的子模块，调用 Sage 算 generator
   - 预期：用 1-2 周时间，可以对 rank=1 的 pair 给出**确定性结论**

2. **配合方向二（Gaussian mod $p^2$ 筛）**
   - 这两个方向没有重叠：方向二在前面砍 pair，方向五在后面判定
   - 组合起来，`concordant` 的诊断速度可以提升一个数量级

3. **方向七、八、十作为长期目标**
   - 不要自己硬啃，找数学系合作者
   - 整理出所有 chain pair 数据 + 已经做出的 concordant 曲线分析，这本身就是合作的资产
