...
- 模块：`src/rational_distance/concordant/heegner_height.py`
- proof pipeline：`proof_status/methods.py::run_heegner_height`
- 方法名仍为：`heegner`

它做的事：

1. 对精确 `rank = 1` 的曲线，用 PARI `ellrank` 给出的 Mordell-Weil
   generator 作为 Heegner generator 的工程替身。
2. 调用 PARI `ellheight` 记录 generator / scanned point 的 canonical height。
3. 枚举 `nG + T`（`T` 为已知 torsion 点，默认 `|n| <= 12`），检查：
   - `X` 是否为正整数平方 `N²`
   - `N² + A²`、`N² + B²` 是否各自为平方
   - 是否满足完整 chain closure
4. 若真的找到 chain-compatible `N`，返回 `solution_found`。
5. 若没找到，**只返回 `inconclusive`**，不会标记 `no_solution`。
...
3. 集成到 `concordant` / `proof_status` workflow 作为 `heegner` method
...
2. **中期目标（2-4 周）**
   ...
   - 工程量：当前已有保守 `run_heegner_height` 诊断版；下一步是补全
     height 上界证明，把部分 `inconclusive` 升级成严格 `no_solution`
