# Rank-Zero Frontier Recheck: 5075,17901 AA

Date: 2026-07-07

## Question

Can the next rank-zero residual frontier target `(5075,17901) AA` be turned into
...
bucket. If Sage closed the elliptic rank bounds to `[0,0]`, the downstream
torsion-preimage audit would need to be rerun under a strict rank-zero proof.
If the rank bounds stay open or the run times out, this target remains on the
proof-work frontier.
...
uv run python scripts/theory/sage_recheck_mixed_closure_residuals.py \
  ...
  --target 5075,17901,AA \
  --second-limit 13 \
  --second-limit 20 \
  --timeout 120 \
  --out results/sage_rankzero_frontier_recheck_5075_17901_AA_s13_20_t120.jsonl
```
...
For `(5075,17901) AA`, the stored status is:
...
This is not a rank-zero proof and not a no-point certificate. It records only
that one finite Sage attempt did not close the rank bounds. The strict partial
