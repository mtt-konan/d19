# Rank-Zero Frontier Recheck: 391,9009 BB

Date: 2026-07-07

## Question

Can the rank-zero residual frontier target `(391,9009) BB` be turned into a
...
bucket. A closed elliptic rank bound `[0,0]` would trigger downstream strict
torsion-preimage auditing. A timeout leaves the target in the proof-work queue.
...
uv run python scripts/theory/sage_recheck_mixed_closure_residuals.py \
  ...
  --second-limit 20 \
  --timeout 120 \
  --out results/sage_rankzero_frontier_recheck_391_9009_BB_s13_20_t120.jsonl
```
...
```text
(391,9009) BB status = timeout
timeout_seconds = 120
elapsed_seconds = 120.003696
...
target_status_counts = {'not-retried': 3, 'sage-timeout': 5}
```

For `(391,9009) BB`, the stored status is:
