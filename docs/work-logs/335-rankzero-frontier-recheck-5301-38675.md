# Rank-Zero Frontier Recheck: 5301,38675 BB

Date: 2026-07-07

## Question

Can the rank-zero residual frontier target `(5301,38675) BB` be turned into a
...
uv run python scripts/theory/sage_recheck_mixed_closure_residuals.py \
  ...
  --target 5301,38675,BB \
  --second-limit 13 \
  --second-limit 20 \
  --timeout 120 \
  --out results/sage_rankzero_frontier_recheck_5301_38675_BB_s13_20_t120.jsonl
```
...
UV_CACHE_DIR=/private/tmp/d19-uv-cache uv run python \
  ...
  --sage-recheck results/sage_rankzero_frontier_recheck_5301_38675_BB_s13_20_t120.jsonl \
...
```text
(5301,38675) BB status = timeout
timeout_seconds = 120
elapsed_seconds = 120.003149
final_rank_bounds = missing
status_counts = {'timeout': 1}
```

The stdout tail reached the first global descent summary:

```text
lower bound = 1
upper bound = 3
second descent started
```

This is only diagnostic text because the script did not return final closed rank
bounds.
...
For `(5301,38675) BB`, the stored status is:
...
This is not a rank-zero proof and not a no-point certificate. The short Sage
retry pass has now covered all 8 rank-zero frontier targets, but none closed.
The strict partial result remains unchanged: only already certified rank-zero
