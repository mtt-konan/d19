...
## 互素腿 concordant N 的 mod-12 律（`coprime_N_div12_check.py`，闭合障碍的模骨架）

把"互素 pair 里没有互素的 N 对"追到底，得到一条更强、更干净的经验律：

> **互素 (A,B) ⟹ 它的每一个 concordant N 都 ≡ 0 (mod 12)**（同时被 3 与 4 整除）。

三套独立数据，**零例外**（合计 >120 万个 concordant N 值）：

| 来源 | 互素 multi-N pair | concordant N 值 | 最大坐标 | N 不被 12 整除 | 互素 N 对 (gcd=1) |
|---|---|---|---|---|---|
...
**推论链（闭合障碍）**：12|Nᵢ 且 12|Nⱼ ⟹ `12 | (Nᵢ±Nⱼ)` ⟹ GEN-CLOSURE 四关系里 N 侧的量
`{Nᵢ+Nⱼ, |Nᵢ−Nⱼ|}` 恒 ≡ 0 (mod 12)。所以**互素腿要出现闭合，必须 `12|(A+B)` 或 `12||A−B|`**。
实测互素对里 A+B 绝大多数是 mod 12 的单位（仅 30/10333 满足 12|(A+B)，且那 30 个 closure 仍=0）——
mod 12 一步排除 ~99.7% 互素对。这是 `chain_closure_sieve` 模筛的显式闭式骨架。

**状态**：经验观察，**尚未证明**（应可用初等 mod 9 / mod 16 Pythagoras 剩余类论证）。**边界**：这条律
是**互素专属**——非互素 (gA',gB') 被 gcd-scaling 破坏（孤岛里见到 gcd=5,7,16… 例外），正是 A.9 §8.6
那个"约化后不可见"的区域。结果存 `results/multi_n/coprime_N_div12_{5M,7Mbfs}.json`。
