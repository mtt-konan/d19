# wl094 — K_9 / K_10 全量 + K_11–K_13 ellrank 审计：rank ≤ 4 假设在 k=9–13 仍严格成立
...
## 延伸：K_11 / K_12 / K_13 hub 直接 ellrank（同 PR 顺手做）

用户要求把审计延伸到 wl085 D-scaling 生成的更高阶 hub。wl085 在 max_value=1M
BFS 范围**之外**用 D-scaling 造出了 K_11–K_13 hub，全部来自 primitive
`(91,990)` (rank 4) 与 `(221,704)` (rank 3) 的放大。

D-scaling 理论 (wl065/wl085)：`E_{a₀,b₀} ≅ E_{d·a₀, d·b₀}` over ℚ ⟹ rank 不变。
所以这里**直接对放大后的大 (a, b) 跑 `compute_rank`**（不是从 primitive 推断），
做双重验证：(1) 放大 hub 的 rank 是否真的等于 primitive rank；(2) rank ≤ 4
是否在 k=11–13 仍成立。

工具 `scripts/partner/k11_k13_ellrank.py`：从 `dscale_kn_smoke.jsonl` 抽 k≥11
唯一 hub（11 个：1 K_13 + 4 K_12 + 6 K_11），逐个 ellrank + closure 检查。

```text
class  scaled pair            k    rank   deficit  prim        prim_rk  match
──────────────────────────────────────────────────────────────────────────────
K_13  ( 458640, 4989600)      13   4      9        (91,990)    4        yes
K_12  ( 152880, 1663200)      12   4      8        (91,990)    4        yes   (4 个 K_12 全同)
K_11  ( 229320, 2494800)      11   4      7        (91,990)    4        yes   (4 个来自 (91,990))
K_11  ( 904995, 2882880)      11   3      8        (221,704)   3        yes   (2 个来自 (221,704))

聚合 (11 个):
  K_13: {4:1}    K_12: {4:4}    K_11: {3:2, 4:4}
  certified (lo==hi):     11/11
  rank == primitive rank: 11/11   ← D-scaling rank 不变性的算术验证
  rank > 4:               0/11
  closure hits:           0/11
```

**结论**：K_11–K_13 全部 rank ≤ 4（rank ∈ {3,4}），且放大 hub 的 rank 精确等于
其 primitive 的 rank（11/11），computationally 确认了 D-scaling 的 rank 不变性。
deficit = k − rank 继续随 k 上升（K_13 deficit=9）。closure 仍 0。

## 合并视图：rank ≤ 4 跨 k=6→13
...
wl060   6–8        11      3–4        catalog 互素行
wl094   9–10       48      3–4        comp 0 非互素 partner-only
wl094   11–13      11      3–4        wl085 D-scaling (BFS 范围外)
──────────────────────────────────────────────────────
合计    6–13       70      3–4        rank ≤ 4 无一例外 (70/70)
```

## 没做的事 / 钩子

1. **E.1（max_value=10M/100M BFS）**：用户已说明 BFS 算法需先优化才能跑更大
   范围，留待后续。
2. **跑全 primitive 库 (854 个 max_hyp≤10000)** 看 K_14+ 是否出现（wl085 §下一步 1），
   再对新 hub 跑 ellrank。
3. rank=4 的 hub 是否有共同的代数特征（如某个固定的 2-descent 像）？可对照
   wl086 的坐标矩阵分析。
...
scripts/partner/k11_k13_ellrank.py                    新: K_11–K_13 直接 ellrank + D-scaling 不变性验证
results/partner/k9k10_ellrank_full.jsonl              48 个 K_9/K_10 hub 全量结果
results/partner/k9k10_ellrank_full_run.log            运行日志
results/partner/k11_k13_ellrank.jsonl                 11 个 K_11–K_13 hub 结果
results/partner/k11_k13_ellrank_run.log               运行日志
