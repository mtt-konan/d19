...
## 四之二、大尺度实证：sum-closure 到 max_hyp=2,000,000

为把「sum 闭合 `N₁+N₂=A+B` 实证失败」的尺度从前述 max_hyp≤2000 推到百万级，复用
pivot-on-N 多-N **生成式**扫描器 `scripts/multi_n/fast_multi_concordant_scan.py`
（`fast_multi_concordant_pairs`：对每个 `A` 因子分解 `A²` 直接还原全部 concordant
`N`，再按 `N` 分组；配合最小质因子线性筛 O(log A)/A）。它只产**约化互素**多-N pair，
正是 §8.6 caveat 覆盖的那一类。

```text
max_hyp=1,000,000   63.3s   111,090 multi-N pair   峰值 5.12 GiB
max_hyp=2,000,000  193.4s   226,120 multi-N pair   峰值 5.64 GiB（见下）
  k 直方图 = {2: 224219, 3: 1814, 4: 83, 5: 4}      ← 首次出现 k=5 saturated pair
  closure pair（N₁+N₂=A+B）= 0
```

**内存说明**：原 `fast_multi_concordant_pairs` 同时持有 `a_sets`（全部 N→A 关系）与
`pairs_with_n`（全部共现 A-对），max_hyp=2e6 时合计 >7.8 GiB，在 8 GiB 机器上被 OOM
杀死（exit 137）。为取得 2M 数据，新增**内存受限变体**
`scripts/multi_n/fast_multi_concordant_scan_lowmem.py`：先丢弃单元素 N-桶，再按 `ai % K`
分 K=8 片生成 pair（每对的较小元 `ai` 固定 ⇒ 分片互斥、不重不漏），峰值降到 5.64 GiB。
该变体在 max_hyp=1e6 上**精确复现** 111,090，结果与库函数一致（193s 含 8 遍桶重扫，CPU
被分片放大；真正的限制是内存而非时间）。

**意义**：到 k=5、22.6 万个约化互素多-N pair，`N₁+N₂=A+B` 仍**全部失败**（0）。这把
§四（正方形内、sum 关系）的无反例实证从 max_hyp=2000 延伸到 **2,000,000**，与 wl046–052
「closure 失败是结构性、非 k 不够」的观察一致（wl052 当年外推「k=5 要 500k–1M 量级」，
此处 2M 实锤 4 个 k=5）。注意这是 **sum 关系**（正方形内）的扩尺度；全平面四关系
GEN-CLOSURE 的扩尺度仍只做到 max_hyp=2000（§四），因 `closure_necessity_relations.py`
另需 safe-pass 过滤，未在此百万级跑。
