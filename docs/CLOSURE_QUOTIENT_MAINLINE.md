...
Lambda convergence priorities audit：
...
Rank-zero isogeny template audit：
...
普通话说：每个本原模型有 3 个 rational 2-torsion kernel；这一步把 243 个模型全部转成
...
Rank-zero Selmer package index export：
...
Rank-zero Selmer isogeny-setup template audit：
...
普通话说：`isogeny_setup` 这一栏也从 9 份平铺收缩成 3 个 kernel 共享模板。
同一个 kernel 下，`symbolic_model` 完全一致；因此后续 transcript 至少有两块已经能复用：
`local_squareclass_conditions` 和 `isogeny_setup`。这仍然只是模板整理，不是 Selmer bound。

Rank-zero Selmer family-conclusion template audit：
...
普通话说：`rank_zero_conclusion` 这一栏不是 9 个互不相干的结论，而是 3 个 family 级聚合结论。
...
普通话说：这一步把每个 `selmer_bound_argument` 再拆成 5 段：共享 isogeny setup 引用、
...
Rank-zero Selmer odd-prime valuation audit：
...
普通话说：这一步把 27 个奇素数小任务继续整理成 valuation 形状：在每个
kernel 和每个 `ell|L`、`ell|T`、`ell|T^2+4L^2` 分支里，记录 `a2`、`a4`
...
Rank-zero Selmer tangent-minus-one non-node branch audit：
...
普通话说：这一步给 9 个 package 加了一道“材料入口”。以后即便有 transcript，也要先通过
...
- `tests/test_closure_quotient_lambda_convergence_priorities.py`
...
- `results/closure_quotient_lambda_convergence_priorities.json`
