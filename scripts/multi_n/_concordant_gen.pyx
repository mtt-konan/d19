...
def generate(long long max_leg):
    ...
    cdef int* spf = <int*> malloc((limit + 1) * sizeof(int))
    if spf == NULL:
        raise MemoryError()
    cdef long long i, j
    ...
    cdef long long* primes = <long long*> malloc(MAXP * sizeof(long long))
    cdef int* exps = <int*> malloc(MAXP * sizeof(int))
    cdef long long dcap = 1 << 16
    cdef long long* divs = <long long*> malloc(dcap * sizeof(long long))
...
def emit_pairs(cnp.ndarray[cnp.int64_t, ndim=1] n_sorted,
    ...
    cdef int pass_id
    cdef long long* buf = <long long*> malloc((maxbucket if maxbucket > 0 else 1) * sizeof(long long))
    if buf == NULL:
        raise MemoryError()
