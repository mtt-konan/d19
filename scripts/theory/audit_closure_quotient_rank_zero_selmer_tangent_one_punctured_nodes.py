#!/usr/bin/env python3
"""Audit tangent-one punctured node-neighborhood squareclass control."""
...
BOUNDARY = (
    "This audits squareclass control on punctured neighborhoods of the "
    "tangent-one nodes by reusing the non-node branch identities. It does not "
...
PUNCTURED_NODE_TARGETS = (
    {
        ...
        "punctured_neighborhood_hypothesis": (
            "X - 1 is nonzero and has positive valuation"
        ),
...
def audit_rank_zero_selmer_tangent_one_punctured_nodes(
    *,
    nonnode_branches: dict[str, Any],
    node_values: dict[str, Any],
) -> dict[str, Any]:
    ...
    for target in PUNCTURED_NODE_TARGETS:
        ...
        if node_entry is None:
            ...
            continue
        if str(nonnode_entry.get("identity", "")) != str(target["identity"]):
            violations.append(f"nonnode_identity_mismatch={schema_id}")
            continue
    ...
    return {
        ...
        "punctured_node_neighborhood_control_proved_count": len(entries),
...
def main() -> int:
    ...
    print(f"wrote rank-zero Selmer tangent-one punctured node audit to {args.out}")
    ...
    print(
        "punctured_node_neighborhood_control_proved_count="
        f"{audit['punctured_node_neighborhood_control_proved_count']}"
    )
    print(
