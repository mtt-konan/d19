#!/usr/bin/env python3
"""Audit tangent-one normal forms for odd-prime local-image schemas."""
...
BOUNDARY = (
    "This audits normal-form reductions for tangent-squareclass-one odd-prime "
    "local-image schemas. It proves only the displayed square-unit coordinate "
    "normalizations; it does not prove local images, local conditions, Selmer "
    "rank bounds, or lambda-family exclusions."
)
...
NEXT_GAP = (
    "compute the 2-isogeny local squareclass image on the standard tangent-one "
    "nodal model"
)
...
def audit_rank_zero_selmer_odd_prime_tangent_one_normal_forms(
    *,
    ...
    unsupported = sorted(
        {
            ...
            for schema in tangent_one_schemas
            if str(schema.get("schema_id", "")) not in NORMAL_FORMS
        }
    )
    if unsupported:
        violations.append(f"unsupported_tangent_one_schemas={unsupported}")
    ...
    return {
        ...
        "tangent_one_schema_count": len(tangent_one_schemas),
...
def main() -> int:
    ...
    print(f"wrote rank-zero Selmer tangent-one normal forms to {args.out}")
    print(f"status={audit['status']}")
    print(f"input_schema_count={audit['input_schema_count']}")
    print(f"tangent_one_schema_count={audit['tangent_one_schema_count']}")
