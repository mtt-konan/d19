#!/usr/bin/env python3
"""Audit tangent-minus-one non-node branch squareclass consequences."""
...
BOUNDARY = (
    "This audits non-node branch squareclass consequences on the "
    "tangent-minus-one standard families. It retains the nonsquare unit "
    "parameter nu and proves only the displayed branch identities; it does not "
    "prove a local image, local condition, Selmer rank bound, or lambda-family "
    "exclusion."
)
...
def audit_rank_zero_selmer_tangent_minus_one_nonnode_branches(
    *,
...
def main() -> int:
    ...
    print(f"wrote rank-zero Selmer tangent-minus-one non-node branches to {args.out}")
