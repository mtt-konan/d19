...
from rational_distance.square import VERTICES, RationalPoint, canonical_xy, compute_distances, make_point
...
class TestParametricSearchFast:
    def test_basic(self):
        results = parametric_search_fast(max_m=20, max_k_num=80, max_k_den=40,
    ...
    def test_inside_only_filters_to_unit_square(self):
        all_pts = parametric_search_fast(max_m=20, max_k_num=80, max_k_den=40,
                                          workers=1, progress=False)
        inside_pts = parametric_search_fast(max_m=20, max_k_num=80, max_k_den=40,
                                             workers=1, progress=False, inside_only=True)
    ...
    def test_min4_returns_subset(self):
        r3 = parametric_search_fast(max_m=20, max_k_num=80, max_k_den=40,
                                     workers=1, progress=False, min_rational=3)
        r4 = parametric_search_fast(max_m=20, max_k_num=80, max_k_den=40,
                                     workers=1, progress=False, min_rational=4)
...
class TestSearchGpuNumpyBackend:
    ...
    def test_basic_results(self):
        pts, backend = parametric_search_gpu(
            max_m=20, max_k_num=80, max_k_den=40,
            min_rational=3, progress=False, xp=np,
        )
    ...
    def test_inside_only(self):
        pts, _ = parametric_search_gpu(
            max_m=20, max_k_num=80, max_k_den=40,
            min_rational=3, progress=False, xp=np, inside_only=True,
        )
        for pt in pts:
            assert Fraction(0) < pt.x < Fraction(1)
            assert Fraction(0) < pt.y < Fraction(1)
    ...
    def test_consistent_with_cpu_path(self):
        ...
        cpu_pts = parametric_search_fast(
            max_m=20, max_k_num=80, max_k_den=40,
            min_rational=3, workers=1, progress=False,
        )
...
class TestEcSearch:
    ...
    def test_returns_rationalpoints(self):
        pts = ec_search(max_m=20, max_k_num=400, max_k_den=800,
                        min_rational=3, progress=False)
        for pt in pts:
            assert pt.rational_count >= 3
    ...
    def test_inside_only_filter(self):
        ...
        from fractions import Fraction as F
        pts = ec_search(max_m=20, max_k_num=400, max_k_den=800,
                        min_rational=3, inside_only=True, progress=False)
        for pt in pts:
