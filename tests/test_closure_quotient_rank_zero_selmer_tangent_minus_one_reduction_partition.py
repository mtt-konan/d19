...
MODULE_NAME = (
    "scripts.theory."
    "audit_closure_quotient_rank_zero_selmer_tangent_minus_one_reduction_partition"
)


def _partition_module() -> ModuleType:
...
def _punctured_nodes() -> dict[str, object]:
    return {
        ...
        "squareclass_contradiction_proved_count": 2,
...
def test_tangent_minus_one_reduction_partition_records_candidate_squareclasses() -> None:
    partition = _partition_module()

    audit = partition.audit_rank_zero_selmer_tangent_minus_one_reduction_partition(
...
def test_tangent_minus_one_reduction_partition_rejects_unready_inputs() -> None:
    partition = _partition_module()
...
def test_tangent_minus_one_reduction_partition_cli_writes_audit(
    tmp_path: Path,
) -> None:
...
def test_write_json_writes_sorted_tangent_minus_one_reduction_partition(
    tmp_path: Path,
) -> None:
    partition = _partition_module()
    out = tmp_path / "reduction_partition.json"

    partition.write_json(out, {"b": 1, "a": 2})
